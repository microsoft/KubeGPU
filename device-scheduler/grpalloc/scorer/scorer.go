...
import (
	...
	resourcefn "github.com/Microsoft/KubeGPU/device-scheduler/grpalloc/resource"
...
func SetScorer(resource types.ResourceName, scorerType int32) ResourceScoreFunc {
	if scorerType == sctypes.DefaultScorer {
		return GetDefaultScorer(resource)
	}
	if scorerType == sctypes.LeftOverScorer {
		return LeftoverScoreFunc
	}
	if scorerType == sctypes.EnumLeftOverScorer {
		return EnumScoreFunc
	}
	return nil
}
